...
Deno.test("step", async (t) => {
  ...
  await t.step("JMI", async (t) => {
    await t.step("normal", async (t) => {
      ...
      for (const { op, adr, gr, fr, expect } of tests) {
        await t.step(
          `op=0x${op.toString(16).padStart(4, "0")}, adr=${
            adr.toString(16).padStart(4, "0")
          }`,
